...
describe("ImportNodeTypes", () => {
    ...
    describe("updateNodeTypesMappingFile()", () => {
        it("should log success to the console", async () => {
            ...
            vi.doMock("../../parser/config/node-types-config.json", () => ({ default: [] }));
            const { updateNodeTypesMappingFile } = await import("./import-node-types.js");
        ...
        it("should log failure to the console", async () => {
            ...
            expect(console.error).toHaveBeenCalledTimes(2);
            expect(console.error).toHaveBeenCalledWith(
                "Error while writing node-types-config.json",
            );
