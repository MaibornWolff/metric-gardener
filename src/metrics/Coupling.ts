    ...
    private getPackages(parseFile: ParseFile) {
        ...
        for (let index = 0; index < textCaptures.length; index+=1) {
            ...
            while(hasClassDeclaration) {
                ...
                packages.set(namespaceName + "\\" + className, {namespace: namespaceName, class: className, source: parseFile.filePath})
    ...
    private getGroupedUsages(parseFile: ParseFile) {
        ...
        for (let index = 0; index < usagesTextCaptures.length; index++) {
            ...
            while (hasUseGroupItem) {
                ...
                usagesOfFile.push({usedNamespace: namespaceName + "\\" + nextUseItem.text, usedSource: parseFile.filePath, alias: "", source: parseFile.filePath})

                hasUseGroupItem = usagesTextCaptures[groupItemIndex + 2]?.name === "namespace_use_item_name"
                groupItemIndex++
                index++
            }
        }
        ...
        return usages.flatMap((usage) => {
            if (packages.has(usage.usedNamespace)) {
                ...
                if (fromData.length > 0) {
                    ...
                    return [{
                        fromNamespace: firstFromNamespace.namespace + "\\" + firstFromNamespace.class,
